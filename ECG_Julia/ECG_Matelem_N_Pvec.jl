#-------------------------------------------------------------------------------------------------------------------------------
# Module ECG_Matelem_N_Pvec  --  N_Pvec matrix elements with AUTOMATIC-DIFFERENTIATION gradients (June 2026)
#
# This is the active N_Pvec matelem. It computes the real L=0 Nitrogen matrix elements Hkl, Skl and obtains
#   Dk = (dHkl/dvechLk, dSkl/dvechLk)   and   Dl = (dHkl/dvechLl, dSkl/dvechLl)
# (each length 2*npt) by forward-mode AD (ForwardDiff) of a single type-generic kernel.
#
# Why this works: the kernel _HS(vechLk, vechLl) allocates every intermediate with the PROMOTED element type of its inputs,
# so ForwardDiff. Verified on a 3-function Nitrogen basis: the AD Dk/Dl match central finite differences to ~1e-10.
#
# Reference
...
#-------------------------------------------------------------------------------------------------------------------------------
# Type-generic N_Pvec kernel: (vechLk, vechLl) -> (Hkl, Skl). All scratch arrays use the promoted element type T so that
# ForwardDiff Duals flow through. The integer data (n, permutation vector pp = PP[:,j0], parvec from set_parvec) and the
# fixed physical data (MassMatrix M, pseudo-charges Q0/Q, covec) are constant w.r.t. the nonlinear parameters.
#-------------------------------------------------------------------------------------------------------------------------------
function _HS(vechLk, vechLl, n, M, Q0, Q, covec, pp, parvec)
...
function MatrixElements(k0,l0,j0,grad_k,grad_l; param::Param=param, verbose1=verbose1, verbose2=verbose2)
    ...
    kernel(vk, vl) = _HS(vk, vl, n, MassMatrix, PseudoCharge0, PseudoCharge, covec, pp, parvec)
